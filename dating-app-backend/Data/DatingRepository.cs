...
namespace dating_app_backend.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            var messages = await _context.Messages.Where(m => m.RecipientId == userId && !m.RecipientDeleted && m.SenderId == recipientId || m.RecipientId == recipientId && m.SenderId == userId && !m.SenderDeleted).OrderByDescending(m => m.MessageSent).ToListAsync();
            return messages;
        }
    }
}